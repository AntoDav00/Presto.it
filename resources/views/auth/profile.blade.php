        ...
        @foreach(['error' => 'danger', 'success' => 'success'] as $msgType => $alertType)
            @if(session()->has($msgType . 'Message'))
                <div class="alert alert-{{ $alertType }} alert-dismissible fade show" role="alert">
                    <i class="bi bi-{{ $alertType == 'danger' ? 'exclamation-triangle' : 'check-circle' }}-fill me-2"></i>
        ...
        <div class="row">
            <div class="col-md-4">
                <div class="profile-img-container position-relative">
                    <img src="{{ Auth::user()->profile_image ? Storage::url(Auth::user()->profile_image) : 'https://www.pngfind.com/pngs/m/676-6764065_default-profile-picture-transparent-hd-png-download.png' }}"
                    ...
                    <form action="{{ route('profile.update-image') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="change-image-overlay btn btn-lg btn-primary">
                            ...
                            <input type="file" id="file-input" class="d-none" name="profile_image" onchange="this.form.submit()"/>
                        </div>
                    </form>
                </div>
            </div>
            <div class="col-md-6">
                <div class="profile-head">
                    ...
                    <a href="{{ route('create') }}" class=" btn log-in-button mt-3">
                        <svg class="me-2" style="width: 1.5vh" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
                    ...
                    <div class="d-md-none mt-3">
                        ...
                        <div class="d-flex flex-column">
                            ...
                            @if (Auth::user()->is_revisor)
                                <a href="{{ route('revisor.index') }}" class="text-decoration-none text-dark fw-bold mb-2">{{__('ui.revisorZone')}}</a>
                            @endif
                    ...
                    <ul class="nav nav-tabs pt-3 pt-md-5" id="myTab" role="tablist">
                        <li class="nav-item">
                            <a class="nav-link active" id="home-tab" data-toggle="tab" href="#home"
                               role="tab" aria-controls="home" aria-selected="true">{{__('ui.info')}}</a>
                        </li>
                        <li class="nav-item">
            ...
            <div class="col-md-2">
                @if (Auth::user()->is_revisor)
                    <a href="{{ route('revisor.index') }}"
                       class="btn btn-success btn-sm position-relative mb-2">
                        {{__('ui.revisorZone')}} 
                    ...
                    <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
                        <div class="user-info">
                            <h4 class="d-flex justify-content-between align-items-center">
                                {{__('ui.userInfo')}}
                                <i class="fas fa-edit text-success edit-profile-btn" style="cursor: pointer;" title="{{__('ui.editProfile')}}"></i>
                            </h4>
                            <div class="row mb-3">
                        ...
                        <form action="{{ route('profile.update') }}" method="POST" class="d-none" id="profileForm">
                            @csrf
                            @method('PUT')
                            <div class="row mb-3">
                                <div class="col-md-6">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-6">
                                    <input type="url" class="form-control" name="facebook" value="{{ Auth::user()->facebook }}">
                                </div>
                            </div>
                            <div class="row mb-3">
                                <div class="col-md-6">
                                ...
                                <div class="col-md-6">
                                    <input type="url" class="form-control" name="linkedin" value="{{ Auth::user()->linkedin }}">
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-12">
                                    <button type="submit" class="btn btn-success">{{__('ui.updateProfile')}}</button>
                                    <button type="button" class="btn btn-danger cancel-edit-btn">{{__('ui.cancel')}}</button>
                                </div>
                            </div>
                        </form>
                    </div>
