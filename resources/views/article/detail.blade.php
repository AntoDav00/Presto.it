        ...
        <div class="row justify-content-center align-items-center">
            <div class="col-12 col-md-4 mb-4">
                @if ($article->images->count() == 0)
                    <img src="{{ asset('img/placeholder.png') }}" class="img-fluid rounded shadow-lg" alt="Placeholder image">
            ...
            <div class="col-12 col-md-6 mt-4 mt-lg-0 d-flex flex-column justify-content-between">
                <div class="p-4 bg-white rounded-4 shadow-lg">
                    ...
                    <p class="fs-2 mb-3 fw-bold text-green"><i class="bi bi-tag-fill me-2"></i>{{ $article->price }}€</p>
                    <p class="lead text-muted">
                        <i class="bi bi-bookmark-fill me-2 text-green"></i><em>{{ __("ui.{$article->category->category}") }}</em>
                    </p>
                    <p class="lead text-muted">
                    ...
                    @auth
                        <form action="{{ route('wishlist.toggle', $article) }}" method="POST">
                            @csrf
                            <button type="submit" class="btn @if(auth()->user()->wishlist->contains($article)) btn-outline-danger @else btn-outline-success @endif mb-3">
                                <i class="bi @if(auth()->user()->wishlist->contains($article)) bi-heart-fill @else bi-heart @endif me-2"></i>
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <p class="mb-0 text-muted"><i class="bi bi-calendar-event me-2 text-green"></i>{{ __('ui.createdAt') }}
                            {{ $article->created_at->format('d/m/Y') }}</p>
                        <p class="mb-0 text-muted"><i class="bi bi-person me-2 text-green"></i>
                            {{ __('ui.seller') }}
                            <a href="{{ route('user.profile', $article->user) }}" class="text-decoration-none text-green">
                                {{ $article->user->name }}
                            </a>
                        </p>
                    </div>
