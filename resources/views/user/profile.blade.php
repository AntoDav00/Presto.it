                ...
                <div class="col-12 mb-5">
                    <div class="card shadow-lg border-0 rounded-lg overflow-hidden">
                        <div class="card-body text-center p-5">
                            ...
                            <p class="text-muted mb-3"><i class="fas fa-calendar-alt mr-2"></i>{{ __('ui.memberSince') }} {{ $user->created_at->format('M Y') }}</p>
                            <p class="mb-3">{{ $user->bio ?? __('ui.noBio') }}</p>
                            <div class="social-links mt-4">
                                @if($user->facebook)
                                    <a href="{{ $user->facebook }}" class="btn btn-outline-primary mx-1"><i class="fab fa-facebook"></i></a>
                                @endif
                                @if($user->linkedin)
                                    <a href="{{ $user->linkedin }}" class="btn btn-outline-primary mx-1"><i class="fab fa-linkedin"></i></a>
                                @endif
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        @forelse ($articles as $article)
                            @if($article->is_accepted)
                                    ...
                                    <x-card :article="$article"  btnlink="{{ route('products.show', compact('article')) }}"/>
                                        </div>
                            @endif
                        @empty
                            <div class="col-12">
